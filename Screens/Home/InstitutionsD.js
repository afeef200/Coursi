...
export default class InstitutionsD extends React.Component {
  constructor(props) {
    super(props);
  ...
  publish = arr => {
    const posts = firebase.database().ref('Rooms/' + this.params.object.key);
    posts.update({
      NumberOfStudent: arr.length - 1,
      Students: arr,
    });
    this.nav('Institutions', {});
    alert('Your Book Has Been Accepted!');
  };

  handlepress = () => {
    if (this.params.object.nums < this.params.object.max) {
      if (this.params.object.students.includes(this.state.username) == true) {
        alert('You have already booked this class!');
        return;
      }
      var arrCopy = this.params.object.students;
      arrCopy.push(this.state.username);
      this.publish(arrCopy);
    } else {
      alert('Class is full!');
    }
  };
